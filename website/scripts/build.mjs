...
function shell({ locale, page, title, description, content, rootPage = false }) {
  ...
  const currentPath = rootPage ? "/" : pagePath(locale, page);
...
for (const locale of locales) {
  ...
  write(path.join(dist, locale.id, "support", "index.html"), shell({ locale, page: "support", title: labels.support, description: labels.intro, content: supportContent(locale, labels) }));
  const privacy = fs.readFileSync(path.join(root, locale.privacy), "utf8");
  write(path.join(dist, locale.id, "privacy", "index.html"), shell({ locale, page: "privacy", title: locale.id === "en" ? "Privacy policy" : "Privacy", description: labels.intro, content: privacyContent(locale, labels, privacy) }));
...
write(path.join(dist, "index.html"), shell({ locale: locales[0], page: "home", title: defaultLabels.home, description: defaultLabels.intro, content: homeContent(locales[0], defaultLabels), rootPage: true }));
